    ...
    function out = isRadioInstalled(obj, aSource)
        if strcmp(aSource, 'RTL-SDR')
            if ~exist('sdrrroot', 'file')
                error('Hardware support package not installed. Open Home > Add-Ons > Get Hardware Support Packages and install Communications Toolbox Support Package for RTL-SDR Radio');
            else
                out = helperFindRTLSDRRadio();
            end
        elseif strcmp(aSource, 'ADALM-PLUTO')
            if isempty(which('plutoradio.internal.getRootDir'))
                error('Hardware support package not installed. Open Home > Add-Ons > Get Hardware Support Packages and install Communications Toolbox Support Package for ADALM-PLUTO Radio');
            else
                 out = helperFindPlutoSDR();
            end
        elseif strcmp(aSource, 'USRP')
            if ~exist('sdruroot', 'file')
                error('Hardware support package not installed. Open Home > Add-Ons > Get Hardware Support Packages and install Communications Toolbox Support Package for USRP Radio');
            else
